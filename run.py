...
from parser import DFG_cpp,DFG_java
...
def convert_examples_to_features(js,tokenizer,args):
    ...
    return InputFeatures(source_tokens,source_ids,position_idx,dfg_to_code,dfg_to_dfg,js['target'])  
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            loss1, logits1 = model(inputs_ids,position_idx,attn_mask,labels)
            loss2, logits2 = model(inputs_ids,position_idx,attn_mask,labels)
            bce_loss = 0.5 * (loss1 + loss2)
            kl_loss = compute_kl_loss(logits1, logits2)
            loss = bce_loss + args.alpha * kl_loss
            if args.n_gpu > 1:
...
def main():
    ...
    parser.add_argument('--alpha', type=int, default=3, help="the parameter for R-Drop.")
    parser.add_argument('--mlp', type=str, default='true', help="Whether to use max pool after the sequence embeddings.")
    parser.add_argument('--lam', type=float, default=0.3, help='the parameter for Contrastive loss.')
    

    args = parser.parse_args()
    
    if str(args.mlp).lower() == 'true':
        args.mlp = True
    else:
        args.mlp = False
